...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0"
  ...
  <xsl:template match="tei:listBibl[tei:bibl/@n]">
    <table>
      <xsl:apply-templates select="@*"/>
      <xsl:call-template name="assign-classes"/>
      <xsl:apply-templates select="tei:bibl" mode="labelled-bibl"/>
    </table>
  </xsl:template>
  <xsl:template match="tei:bibl">
    <xsl:call-template name="make-p"/>
  </xsl:template>
  <xsl:template match="tei:p[not(contains(concat(' ', @nzetc:class, ' '), ' block '))]//tei:bibl">
    <xsl:apply-templates/>
  </xsl:template>
  ...
  <xsl:template match="tei:name[@key][contains(concat(' ', @nzetc:class, ' '), ' nested-link ')] |
      ...
      <xsl:choose>
        <xsl:when test="$target/self::tei:note">
          <xsl:variable name="excerpt">
            <xsl:apply-templates select="$target" mode="hyperlink-title"/>
          </xsl:variable>
          <xsl:if test="$excerpt != .">
        ...
        <xsl:when test="$target/self::tei:div or $target/self::tei:figure">
          <xsl:variable name="excerpt">
            <xsl:apply-templates select="$target/tei:head[1]"
                                 mode="hyperlink-title"/>
          </xsl:variable>
          <xsl:if test="$excerpt != .">
            <xsl:value-of select="normalize-space($excerpt)"/>
          </xsl:if>
  ...
  <xsl:template match="tei:ref[contains(concat(' ', @nzetc:class, ' '), ' nested-link ')]" priority="1000">
    <xsl:apply-templates select="node()"/>
  </xsl:template>
  ...
  <xsl:template match="tei:figure">
    ...
    <xsl:element name="{$container-element}">
      ...
      <!-- Use a scaled-down version of the image, with a link to the
           full-size image. -->
      <xsl:variable name="root-id" select="/tei:TEI/@xml:id | /tei:teiCorpus/@xml:id"/>
        ...
        <xsl:otherwise>
          ...
          <xsl:choose>
            <xsl:when test="$nested-link">
              <img src="/etexts/{$root-id}/{substring-before(tei:graphic/@url, '.')}({$scale}).{substring-after(tei:graphic/@url, '.')}">
                <xsl:apply-templates select="." mode="alt-text"/>
              </img>
            </xsl:when>
            <xsl:otherwise>
      ...
      <xsl:choose>
        <xsl:when test="@type='advert' or @n='advert' or @type='adverts' or @n='adverts' or
    ...
    <xsl:choose>
      <xsl:when test="number($pagenumbers)">
        <span class="pb" lang="en">
          ...
          <xsl:choose>
            <xsl:when test="not(contains(concat(' ', @nzetc:class, ' '), ' nested-link ')) and @xml:id">
              <a class="pb" href="#{@xml:id}" title="page break">
        ...
        <xsl:if test="@corresp">
          ...
          <xsl:apply-templates select="/tei:TEI/tei:teiHeader/tei:fileDesc/tei:notesStmt/tei:note[@xml:id='page-images']//tei:figure[@xml:id=$figure-id]">
            <xsl:with-param name="nested-link" select="contains(concat(' ', @nzetc:class, ' '), ' nested-link ')"/>
          </xsl:apply-templates>
        </xsl:if>
      </xsl:when>
      <xsl:when test="@xml:id">
        <span class="pb" id="{@xml:id}"/>
      </xsl:when>
    </xsl:choose>
  </xsl:template>
      ...
      <span>
        ...
        <span class="sic-mark">
          ...
          <xsl:choose>
            ...
            <xsl:when test="$corrected-text">
              <xsl:text>: </xsl:text>
              <xsl:apply-templates select="$corrected-text" mode="corrections"/>
            </xsl:when>
            <xsl:otherwise/>
          </xsl:choose>
          <xsl:text>]</xsl:text>
  ...
  <xsl:template match="tei:orig">
    <xsl:if test="not(number($regularised))">
      <span>
        ...
        <xsl:attribute name="title">
          <xsl:apply-templates select="../tei:reg" mode="regularised"/>
        </xsl:attribute>
        <xsl:apply-templates select="@*"/>
        <xsl:call-template name="assign-classes"/>
        <xsl:apply-templates select="node()"/>
  ...
  <xsl:template match="tei:gap">
    <span lang="en">
      ...
      <xsl:call-template name="assign-classes"/>
      <xsl:text>[</xsl:text>
      <span title="Material has been omitted from the transcription at this point." class="head">gap &#8212; </span>
  ...
  <xsl:template match="tei:hi[@rend]">
    <xsl:variable name="class">
      <xsl:apply-templates select="@rend" mode="rend-class"/>
    </xsl:variable>
    <span>
  ...
  <xsl:template name="make-run-in-heading">
    <xsl:variable name="run-in-heading" select="preceding-sibling::*[1]/self::tei:head[contains(@rend, 'run-in')]"/>
  ...
  <xsl:template name="assign-classes">
    ...
    <xsl:variable name="classes">
      ...
      <xsl:apply-templates select="$node/@rend" mode="rend-class"/>
    ...
    <xsl:variable name="styles">
      <xsl:apply-templates select="$node/@rend" mode="rend-style"/>
    </xsl:variable>
